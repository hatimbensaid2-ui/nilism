    ...
    <div className="p-6 max-w-2xl">
      ...
      <form onSubmit={handleSave} className="space-y-5">
        ...
        <Section title="Customer Messaging">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Lookup Page Intro Text</label>
            <input value={form.lookupIntroText || ''} onChange={f('lookupIntroText')}
              placeholder="Enter your order details to get started."
              className="w-full px-3.5 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent" />
            <p className="text-xs text-gray-400 mt-1">Shown below "Returns Center" heading on the lookup page.</p>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Order Number Field Label</label>
              <input value={form.orderNumberLabel || ''} onChange={f('orderNumberLabel')}
                placeholder="Order number"
                className="w-full px-3.5 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Email Field Label</label>
              <input value={form.emailLabel || ''} onChange={f('emailLabel')}
                placeholder="Email"
                className="w-full px-3.5 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent" />
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Find Order Button Text</label>
            <input value={form.findOrderText || ''} onChange={f('findOrderText')}
        ...
        <Section title="Return Policy">
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Return Window Start</label>
              <div className="flex gap-3">
                {[
                  { value: 'fulfilled', label: 'Fulfillment date', desc: 'Window starts when Shopify marks the order as fulfilled (shipped).' },
                  { value: 'delivered', label: 'Delivery date', desc: 'Window starts when the carrier marks the package as delivered. Requires tracking.' },
                ].map(opt => (
                  <label key={opt.value}
                    className={`flex-1 flex items-start gap-2.5 p-3 rounded-xl border cursor-pointer transition-colors ${form.returnWindowFrom === opt.value ? 'border-indigo-400 bg-indigo-50' : 'border-gray-200 hover:border-gray-300'}`}>
                    <input type="radio" name="returnWindowFrom" value={opt.value}
                      checked={form.returnWindowFrom === opt.value}
                      onChange={() => setForm(prev => ({ ...prev, returnWindowFrom: opt.value }))}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Return Window Duration</label>
              <div className="flex items-center gap-2">
              ...
              <p className="text-xs text-gray-400 mt-1">Orders past this window will show as ineligible.</p>
            </div>
          </div>
        </Section>
